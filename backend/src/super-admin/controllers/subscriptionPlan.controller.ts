...
export async function getSubscriptionPlans(req: Request, res: Response, next: NextFunction) {
  try {
...
export async function createSubscriptionPlan(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (checkError) {
      logger.error('Error checking duplicate subscription plan', { error: checkError });
...
export async function updateSubscriptionPlan(req: Request, res: Response, next: NextFunction) {
  try {
    const { id } = req.params;
    const { name, description, price, amc_price, duration_days, is_active } = req.body;
    ...
    if (checkError) {
      logger.error('Error checking duplicate subscription plan update', { error: checkError });
      return res.status(500).json({ ok: false, message: 'Database check failed' });
    }
...
export async function toggleSubscriptionPlanStatus(req: Request, res: Response, next: NextFunction) {
  try {
    const { id } = req.params;
