...
async function getCities(req, res, next) {
    try {
        ...
        if (error) {
            logger_1.default.error('Failed to fetch cities', { error });
...
async function createCity(req, res, next) {
    try {
        ...
        if (checkError) {
            logger_1.default.error('Error checking duplicate city', { error: checkError });
        ...
        const { data, error } = await supabaseClient_1.default
            ...
            .single();
        if (error) {
            logger_1.default.error('Failed to create city', { error });
            return res.status(500).json({ ok: false, message: 'Failed to create city' });
        }
        const formattedData = {
            ...data,
            state_name: data.states?.name || 'Unknown',
            country_name: data.states?.countries?.name || 'Unknown',
        };
...
async function updateCity(req, res, next) {
    try {
        ...
        const { data: existing, error: checkError } = await supabaseClient_1.default
            ...
            .ilike('name', name.trim());
        if (checkError) {
            logger_1.default.error('Error checking duplicate city update', { error: checkError });
            return res.status(500).json({ ok: false, message: 'Database check failed' });
        }
        ...
        const { data, error } = await supabaseClient_1.default
            ...
            .single();
        if (error) {
            logger_1.default.error('Failed to update city', { error });
...
async function toggleCityStatus(req, res, next) {
    try {
        const { id } = req.params;
        ...
        const { data, error } = await supabaseClient_1.default
            ...
            .single();
        if (error) {
            logger_1.default.error('Failed to toggle city status', { error });
