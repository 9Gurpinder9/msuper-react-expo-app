...
export default function CompanyResetPassword() {
    ...
    useCallback(() => {
      ...
      (async () => {
        const [storedEmail, storedToken, stage] = await Promise.all([
          ...
          AsyncStorage.getItem('companyResetToken'),
  ...
  const handleResetPassword = async () => {
    ...
    const url = `${API_BASE_URL}/company/auth/reset-password/confirm`;
    try {
      ...
      logger.error('Company reset password network exception', { message: msg, url });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} disabled={Platform.OS === 'web'}>
      <KeyboardAvoidingView style={styles.wrapper} behavior={Platform.select({ ios: 'padding' })}>
        <TopAppBar title="Set New Password" showBack onBackPress={() => router.replace('/company/login')} />
