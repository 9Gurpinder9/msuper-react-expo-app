...
export default function CompanyResetOtp() {
    ...
    useCallback(() => {
      ...
      (async () => {
        const [storedEmail, stage, expiresAtRaw] = await Promise.all([
          AsyncStorage.getItem('companyResetEmail'),
          AsyncStorage.getItem('companyResetStage'),
          AsyncStorage.getItem('companyResetOtpExpiresAt'),
        ]);
        if (!active) return;
        if (!storedEmail) {
  ...
  const handleVerify = async () => {
    ...
    const url = `${API_BASE_URL}/company/auth/reset-password/verify-otp`;
    try {
      ...
      setVerified(true);
      if (body.resetToken) {
        await AsyncStorage.setItem('companyResetToken', body.resetToken);
        await AsyncStorage.setItem('companyResetStage', 'confirm');
      }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        await AsyncStorage.setItem('companyResetOtpExpiresAt', expiresAt);
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <KeyboardAvoidingView style={StyleSheet.absoluteFill} behavior={Platform.select({ ios: 'padding' })}>
        <TopAppBar title="Verify Reset OTP" showBack onBackPress={() => router.replace('/company/forgot-password')} />
