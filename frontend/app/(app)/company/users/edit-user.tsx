...
export default function EditUserPage() {
  ...
  // Disable logic if target is ADMIN role
  const isTargetAdmin = String(params.role_name || '').toUpperCase() === 'ADMIN';

  // Form Fields
  const [formName, setFormName] = useState(String(params.name || ''));
  const [formEmail, setFormEmail] = useState(String(params.email || ''));
  const [formMobile, setFormMobile] = useState(String(params.mobile || ''));
  const [formAddress, setFormAddress] = useState(String(params.address || ''));
  const [formIsActive, setFormIsActive] = useState(params.is_active === 'true');

  // Selected values
  const [selectedRole, setSelectedRole] = useState<Option | null>(
    params.role_id ? { id: Number(params.role_id), name: String(params.role_name || '') } : null
  );
  const [selectedCountry, setSelectedCountry] = useState<Option | null>(
    params.country_id ? { id: Number(params.country_id), name: String(params.country_name || '') } : null
  );
  const [selectedState, setSelectedState] = useState<Option | null>(
    params.state_id ? { id: Number(params.state_id), name: String(params.state_name || '') } : null
  );
  const [selectedCity, setSelectedCity] = useState<Option | null>(
    params.city_id ? { id: Number(params.city_id), name: String(params.city_name || '') } : null
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetchJson(`${API_BASE_URL}/company/users/${userId}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        showError(response.data?.message || 'Failed to update user details.');
        setSaving(false);
        return;
      }

      const statusResponse = await fetchJson(`${API_BASE_URL}/company/users/${userId}/status`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (statusResponse.ok) {
        ...
        showError(statusResponse.data?.message || 'Failed to update status.');
      }
    } catch {
      showError('Network error updating user.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <View style={styles.wrapper}>
      <TopAppBar
        title={isTargetAdmin ? "View ADMIN Details" : "Edit Team Member"}
        showBack
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={[styles.formCard, isLargeScreen && { alignSelf: 'center', width: '70%', maxWidth: 700 }]}>
          <Text variant="titleLarge" style={styles.formTitle}>
            {isTargetAdmin ? "ADMIN Details (Read Only)" : "Edit Member Details"}
          </Text>
          ...
          <View style={{ gap: 16 }}>
            ...
            <Field label="State" required error={fieldErrors.state}>
              <SelectorTrigger
                ...
                disabled={isTargetAdmin || !selectedCountry}
                onPress={openStates}
                theme={theme}
              />
            </Field>
            ...
            <Field label="City" required error={fieldErrors.city}>
              <SelectorTrigger
                ...
                disabled={isTargetAdmin || !selectedState}
                onPress={openCities}
                theme={theme}
              />
            </Field>
      ...
      {saving && (
        <Portal>
          <View style={styles.loaderBackdrop}>
            <AppLoader message="Saving user details..." icon="database-sync-outline" transparent />
          </View>
        </Portal>
      )}
    </View>
