// frontend/app/(app)/company/bookmarks/add-bookmark.tsx
...
import { createBookmark } from '../../../../src/features/bookmarks/api';
...
export default function AddBookmark() {
  ...
  const openCategorySelector = useCallback(async () => {
    ...
    if (catCache.current) {
      setCatOptions(catCache.current);
      return;
    }
    setCatLoading(true);
    try {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showError(err?.message || 'Failed to create bookmark');
    } finally {
      setSaving(false);
    }
  };
      ...
      {saving && (
        <Portal>
          <View style={[StyleSheet.absoluteFillObject, { zIndex: 9999, backgroundColor: theme.dark ? 'rgba(0,0,0,0.65)' : 'rgba(255,255,255,0.75)', justifyContent: 'center', alignItems: 'center' }]}>
            <AppLoader message="Saving bookmark..." icon="database-sync-outline" transparent />
          </View>
        </Portal>
      )}
    </View>
