// frontend/app/(app)/company/bookmark-categories/add-bookmark-category.tsx
...
export default function AddBookmarkCategory() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showSuccess('Bookmark category created');
      router.back();
    } catch (err: any) {
      showError(err?.message || 'Failed to create bookmark category');
    } finally {
      setSaving(false);
    }
  };
      ...
      {saving && (
        <Portal>
          <View style={[StyleSheet.absoluteFillObject, { zIndex: 9999, backgroundColor: theme.dark ? 'rgba(0,0,0,0.65)' : 'rgba(255,255,255,0.75)', justifyContent: 'center', alignItems: 'center' }]}>
            <AppLoader message="Saving bookmark category..." icon="database-sync-outline" transparent />
          </View>
        </Portal>
      )}
    </View>
