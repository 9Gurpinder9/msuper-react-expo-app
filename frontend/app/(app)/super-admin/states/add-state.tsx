// frontend/app/(app)/super-admin/states/add.tsx
...
export default function AddState() {
  ...
  const handleSave = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('authToken');
      const url = `${API_BASE_URL}/super-admin/states`;
      ...
      if (response.ok) {
        showSuccess(response.data?.message || 'State registered successfully');
        router.back();
      } else {
      ...
      {saving && (
        <Portal>
          <View style={[StyleSheet.absoluteFillObject, { zIndex: 9999, backgroundColor: theme.dark ? 'rgba(0,0,0,0.65)' : 'rgba(255,255,255,0.75)', justifyContent: 'center', alignItems: 'center' }]}>
            <AppLoader message="Registering new state..." icon="database-sync-outline" transparent />
          </View>
        </Portal>
      )}
    </View>
