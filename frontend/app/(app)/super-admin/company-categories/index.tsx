// frontend/app/(app)/super-admin/company-categories/index.tsx
...
export default function CompanyCategoriesRegistry() {
  ...
  const fetchCategories = async (query = '', pageNum = 1, isLoadMore = false) => {
    if (pageNum === 1) {
      setLoading(true);
    } else {
      setLoadingMore(true);
    ...
    try {
      ...
      const baseUrl = `${API_BASE_URL}/super-admin/company-categories`;
      ...
      if (response.ok && response.data) {
        ...
        showError(response.data?.message || 'Failed to load company categories');
      }
    } catch {
      showError('Network error loading company categories');
    } finally {
      setLoading(false);
  ...
  const handleLoadMore = () => {
    ...
    fetchCategories(search, page + 1, true);
  };
  ...
  const handleOpenAdd = () => {
    router.push('/super-admin/company-categories/add-category');
  };

  const handleOpenEdit = (item: CompanyCategory) => {
    router.push({
      pathname: '/super-admin/company-categories/edit-category',
      params: {
        id: item.id,
        name: item.name,
  ...
  function renderTableItem({ item, index }: { item: CompanyCategory; index: number }) {
    return (
      <View style={styles.tableRow}>
        ...
        <View style={[styles.tableCell, styles.cellAction, { borderRightWidth: 0, alignItems: 'center' }]}>
          <Button
            ...
            testID={`edit-category-button-${index}`}
          >
            Edit
          </Button>
        </View>
  ...
  function renderListItem({ item, index }: { item: CompanyCategory; index: number }) {
