...
export default function AddFeaturePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/super-admin/features`;
      ...
      if (response.ok) {
        showSuccess('Feature registered successfully');
        router.back();
      } else {
        showError(response.data?.message || 'Failed to register feature.');
      }
    } catch {
      showError('Network error registering feature.');
    } finally {
      setSaving(false);
    }
  };
      ...
      {saving && (
        <Portal>
          <View style={[StyleSheet.absoluteFillObject, { zIndex: 9999, backgroundColor: theme.dark ? 'rgba(0,0,0,0.65)' : 'rgba(255,255,255,0.75)', justifyContent: 'center', alignItems: 'center' }]}>
            <AppLoader message="Registering new feature..." icon="database-sync-outline" transparent />
          </View>
        </Portal>
      )}
    </View>
