// frontend/app/(app)/super-admin/roles/edit.tsx
...
export default function EditRolePage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Load role metadata and current permissions
      if (roleId) {
  ...
  const handleSave = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('authToken');
      const url = `${API_BASE_URL}/super-admin/roles/${roleId}`;
      ...
      if (response.ok) {
        ...
        const permRes = await fetchJson(`${API_BASE_URL}/super-admin/roles/${roleId}/permissions`, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
    ...
    <View style={styles.wrapper}>
      ...
      {saving && (
        <View style={[StyleSheet.absoluteFillObject, { zIndex: 9999, backgroundColor: theme.dark ? 'rgba(0,0,0,0.65)' : 'rgba(255,255,255,0.75)', justifyContent: 'center', alignItems: 'center' }]}>
          <AppLoader message="Updating role settings..." icon="database-sync-outline" transparent />
        </View>
      )}
    </View>
  );
