// frontend/app/(app)/super-admin/cities/edit.tsx
...
export default function EditCity() {
  ...
  const [stateId, setStateId] = useState(params.stateId || '');
  const [stateName, setStateName] = useState(params.stateName || '');
  ...
  const handleSave = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('authToken');
      const url = `${API_BASE_URL}/super-admin/cities/${params.id}`;
      ...
      if (response.ok) {
        showSuccess(response.data?.message || 'City updated successfully');
        router.back();
      } else {
      ...
      {saving && (
        <Portal>
          <View style={[StyleSheet.absoluteFillObject, { zIndex: 9999, backgroundColor: theme.dark ? 'rgba(0,0,0,0.65)' : 'rgba(255,255,255,0.75)', justifyContent: 'center', alignItems: 'center' }]}>
            <AppLoader message="Updating city details..." icon="database-sync-outline" transparent />
          </View>
        </Portal>
      )}
    </View>
