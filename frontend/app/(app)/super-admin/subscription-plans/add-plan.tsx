// frontend/app/(app)/super-admin/subscription-plans/add.tsx
...
export default function AddSubscriptionPlan() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/super-admin/subscription-plans`;
      ...
      if (response.ok) {
        showSuccess('Plan registered successfully');
        router.back();
      } else {
      ...
      {saving && (
        <Portal>
          <View style={[StyleSheet.absoluteFillObject, { zIndex: 9999, backgroundColor: theme.dark ? 'rgba(0,0,0,0.65)' : 'rgba(255,255,255,0.75)', justifyContent: 'center', alignItems: 'center' }]}>
            <AppLoader message="Registering new plan..." icon="database-sync-outline" transparent />
          </View>
        </Portal>
      )}
    </View>
